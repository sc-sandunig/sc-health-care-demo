...
const StrippedBlobAccent = ({ variant = 'blob', ...props }) => {
  const clipId = `clippath-${variant}`;
  const clipIdInner = `${clipId}-1`;

  const renderClipPath = () => {
    if (variant === 'circle') {
      return (
        <clipPath id={clipId}>
          <circle cx="232.76" cy="231.04" r="230" />
        </clipPath>
      );
    }

    // Default to blob shape
    return (
      <clipPath id={clipId}>
        <path
